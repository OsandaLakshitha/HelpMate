...
export default function SlotsPage() {
  ...
  const handleSave = async (slotName) => {
    ...
    if (!pref) return;
          if (hasOverlap(localPrefs, slotName, pref.start_time, pref.end_time)) return
    ...
    try {
      ...
      await savePreference({
        ...
        max_pomodoros:
          calcMaxPomos(pref.start_time, pref.end_time) ?? pref.max_pomodoros,
...
return mins > 0 ? Math.min(Math.floor(mins / 25), 48) : null;
  };
...
const hasOverlap = (slots, currentSlotName, start, end) => {
  const s1 = toMins(start)
  const e1 = toMins(end)
  if (s1 === null || e1 === null || s1 >= e1) return false
  return slots
    .filter(p => p.slot_name !== currentSlotName && p.start_time && p.end_time)
    .some(p => {
      const s2 = toMins(p.start_time)
      const e2 = toMins(p.end_time)
      return s1 < e2 && e1 > s2   // standard interval overlap formula
    })
}
        ...
        return (
          ...
          >
            ...
            <div className="space-y-3">
              <div>
                <label className="text-xs text-masss-heading/50 mb-1 block">
                  Display label
                </label>
                <input
                  type="text"
                  ...
                  onChange={(e) =>
                    update(slotName, "slot_label", e.target.value)
                  }
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      update(slotName, "start_time", e.target.value)
                    }
                    className="w-full px-3 py-2 rounded-lg border border-masss-mint text-sm text-masss-heading bg-masss-bg focus:outline-none focus:border-masss-accent"
                  />
                </div>
                <div>
                  ...
                  <input
                    ...
                    value={pref.end_time || ""}
                    onChange={(e) =>
                      update(slotName, "end_time", e.target.value)
                    }
                    className="w-full px-3 py-2 rounded-lg border border-masss-mint text-sm text-masss-heading bg-masss-bg focus:outline-none focus:border-masss-accent"
                  />
                </div>
              </div>
               {hasOverlap(localPrefs, pref.slot_name, pref.start_time, pref.end_time) && (
                <p className="text-[10px] text-red-500">
                  ⚠ This time range overlaps with another slot
                </p>
              )}
              <div>
                <div className="flex items-center justify-between mb-1">
                  <label className="text-xs text-masss-heading/50">
                    Max pomodoros for this slot
                  </label>
                  <span className="text-[10px] font-semibold text-masss-accent">
                    Slot max{" "}
                    {calcMaxPomos(pref.start_time, pref.end_time) ?? "—"} 🍅
                  </span>
                </div>
                <input
                  type="number"
