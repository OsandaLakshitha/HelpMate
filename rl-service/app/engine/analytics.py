...
class UserAnalyticsService:
    ...
    def _session_in_slot(self, session: dict, start: int, end: int) -> bool:
        try:
            ...
            # Strip timezone info so comparison works regardless of
            # whether the ISO string came with a Z/+00:00 suffix or not
    ...
    def _signal_session_streaks(self, sessions: List[dict]) -> float:
        ...
        try:
            ...
            sorted_s = sorted(sessions, key=parse)
        except (KeyError, ValueError):
            return 3.0
        ...
        for i in range(len(sorted_s) - 1):
            try:
                ...
                t1 = parse_dt(sorted_s[i])
                t2 = parse_dt(sorted_s[i + 1])
    ...
    def _get_recent_performance_history(self) -> List[float]:
        def safe_started_at(s):
            try:
                dt = datetime.fromisoformat(s.get("started_at", ""))
